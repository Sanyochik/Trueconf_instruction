...
// TrueConf server ip или адрес (сслка на trueconf server)
const server = "https://45.147.178.90";

// OAuth2 приложение ID клиента
const client_id = "client_id_OAuth2";

// OAuth2 приложение секретный ключ
const client_secret = "client_secret_OAuth2";
...
const wait_for_answer = 10; // в секундах
const wait_before_delete = 1; // в секундах
...
async function messageListener(msg){
	...
	switch(msg.data.type){
		case "notSupported":
			showError("WebRTC не поддерживается.");
			await sleep(deleteConferenceByTimer, wait_before_delete);
			break;
		case "loginFail":
			showError("Не верный id или secret.");
			await sleep(deleteConferenceByTimer, wait_before_delete);
			break;
		case "Подключение закрыто":
			await sleep(deleteConferenceByTimer, wait_before_delete);
			break;
	}
}
...
async function startCall(){
	try{
		...
		if(!result_status && !cancel_call){
			cancelTimer();
			showError("Не удалось найти свободных операторов. Пожалуйста, повторите попытку позже.");
		}
		else{
		}
	}
...
async function createConferenceObject(){
	...
	let conference_object = '{ "topic": "Вызов менеджера", "type":0, "auto_invite": 0, "max_participants": 2, "invitations":[{"id":"' +
...
async function getWebrtcUrl(){
	...
	if (webRTCdata.length == 0)
		throw new Error("Не удалось найти ссылку на конференцию. Проверьте пожалуйста вашу лицензию.");
...
async function getJsonResponse(response){
	if (response.ok){
		...
		throw new Error("Ошибка при получении данных " + response.status + ", сообщение ошибки: '" + response.message + "'");
	}
}
...
function checkApiError(json_data){
	if("error" in json_data){
		...
		throw new Error("Запрос вернул ошибку '" + json_data.error + "' с сообщением: '" + json_data.error_description + "'");
	}
}
...
function getGroupByName(groups, group_name){
	...
	if (i == groups.length){
		throw new Error("Не удалось найти группу 'operators'.");
	}
}
...
function showConferenceFrame(webrtc_url){
	...
	conference_frame.style.width = "1800px";
	conference_frame.style.height = "750px";
